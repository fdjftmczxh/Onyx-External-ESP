...
namespace android
{
    ...
    ATouchEvent::ATouchEvent()
    {
        ...
        for (const auto &devicePath : std::filesystem::directory_iterator("/dev/input"))
        {
            ...
            if (ioctl(m_deviceFd, EVIOCGNAME(sizeof(deviceName)), deviceName) < 0) {
                // Ignore error
            }

            LogInfo("[*] Checking device: %s [%s] [BTN_TOUCH: %d, BTN_TOOL_FINGER: %d]", 
                    devicePath.path().c_str(), deviceName, m_keyBitmask.test(BTN_TOUCH), m_keyBitmask.test(BTN_TOOL_FINGER));

            if (m_keyBitmask.test(BTN_TOUCH) || m_keyBitmask.test(BTN_TOOL_FINGER)) {
                LogInfo("[+] Found touch device: %s", devicePath.path().c_str());
                break;
            }
        ...
        LogInfo("[+] Touch Scalers: X=%d, Y=%d", transformScalerX, transformScalerY);
    }
    ...
    bool ATouchEvent::GetTouchEvent(TouchEvent *touchEvent)
    {
        ...
        while (true)
        {
            ssize_t readResult = read(m_deviceFd, &event, sizeof(input_event));
            if (0 >= readResult)
                return false;

            // Check if event is not a submit event
            if (EV_SYN != event.type || SYN_REPORT != event.code)
            {
                cachedEventQueue.push_back(event);
                continue; // Keep reading raw events from the buffer
            }

            // We hit a SYN_REPORT, assemble the packet
            if (cachedEventQueue.empty())
                return false;
